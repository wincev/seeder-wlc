...
public:
  ...
  void ParseCommandLine(int argc, char **argv) {
    static const char *help = "WorldLeadCurrency-seeder\n"
...
static const string mainnet_seeds[] = {"dnsseed.wlc-dnsseed.ssdpool.com",""};
...
extern "C" void* ThreadSeeder(void *arg) {
  ...
  if (!fTestNet){
    db.Add(CService(opts->ns, 55889, true), true);
  }
  do {
...
int main(int argc, char **argv) {
  ...
  if (opts.tor) {
    CService service(opts.tor, 55889);
    if (service.IsValid()) {
